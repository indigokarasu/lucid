...
=======
- **read_file path resolution**: The `read_file` tool may fail with "File not found" for absolute profile paths (e.g., `~/.hermes/skills/ocas-lucid/references/foo.md`). Use `read_file` with the expanded `~` path (`~/.hermes/profiles/indigo/skills/ocas-lucid/references/foo.md`) or fall back to `terminal(head <path>)`.
>>>>>>> Stashed changes
